...
export const WhitepaperQs = ({title, children}) => (
  ...
  export const ParagraphComponent = ({content, children, white, minimal, moreMinial}) => {
    return (
      <p className={`${ white && 'text-[#d6d6dc]'} text-[18px] md:text-[20px] ${moreMinial && 'py-1 md:py-2'} ${minimal && 'py-3 md:py-4'} py-6 md:py-8`}>
          <span>{children}</span>
          {content}
      </p>
    )
